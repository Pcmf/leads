...
angular.module('appGest').controller('gdashboardController',function($scope,$http,$interval,$rootScope){
   ...
   $scope.puxarLead = function() {
        if($scope.userData.presenca==0) {
            if(($scope.agendaAtiva + $scope.ativa + $scope.agendaDoc)>0){
                sessionStorage.turn = "A";
                window.location.replace("#!/gcontact");
            } else { 
                alert("Não é permitido puxar leads novas");
            }
        } else {
            window.location.replace("#!/gcontact");
        }
...
//    // Puxar lead de recuperação
//    $scope.puxarLeadRec = function() {
//            window.location.replace("#!/gcontactrec");
//    }
