 ...
 */
angular.module('appGest').controller('gcontactController', function ($scope, $http, $modal, $routeParams) {
    $scope.c = {};
    $scope.c.mkCall = true;
    $scope.tpc = 'CP';
    ...
    if ($routeParams.id) {
        ...
        $http({
            url: 'php/gestor/getPushedLead_1.php',
            method: 'POST',
        ...
        $http({
            ...
            // Se a lead tiver status > 1 redireciona para o detalhe
            $scope.lead = answer.data.lead;
            if (answer.data.goToDet) {
