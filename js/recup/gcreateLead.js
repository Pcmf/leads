 ...
 */
angular.module('appRec').controller('gcreateLeadController',function($scope,$http,$modal){
    $scope.newLeadId = 0;
    $scope.c ={};
    ...
    $scope.attachDocs = function(){
        if(checkFields()){
            ...
            if(!$scope.newLeadId){
            $http({
                url:'php/recup/saveFormData.php',
                method:'POST',
 ...
 */
angular.module('appRec').controller('modalInstanceFinalizeNew', function($scope,$http,$modal,$modalInstance,items){
    ...
    //    alert(m.dataExpectavel);
        if(d.docs==undefined || d.docs.length==0){
            ...
            $http({
                url:'php/recup/saveNewLeadProcess.php',
                method:'POST',
 ...
 */
angular.module('appRec').controller('modalInstanceAttachDocsD', function($scope,$http,$modal,$modalInstance,items){
     $scope.d = {};
     $scope.da ={};
     $scope.lead = items;
     getInf();
 //Anexar documento
    ...
    $scope.aguardarDoc = function(lead){
        ...
        $http({
            url:'php/gestor/changeLeadStatus.php',
            method:'POST',
            data:JSON.stringify({'lead':lead,'status':108,'user':JSON.parse(sessionStorage.userData)})
        }).then(function(answer){
            $modalInstance.close('ok');
        });
    };
 ...
 */
angular.module('appRec').controller('modalInstanceAnexarDocD', function($scope,$http,$modalInstance,items){
 ...
 */
angular.module('appRec').controller('modalInstanceAnexarDocExtraD', function($scope,$http,$modalInstance,items){
    $scope.lead = items;
    $scope.novonome ="";
    $scope.d = {};
