...
angular.module('appRec').controller('detLeadRecController', function ($scope, $http, $routeParams, $modal) {
    ...
    $scope.removeLnOR = function(ln) {
        ...
        $http({
                ...
                data: JSON.stringify({ 'or': ln})
                    ...
                    $scope.rendimentos = $scope.rendimentos.filter((el) => {return el.linha != ln.linha } );
    ...
    $scope.removeLnOC = function(ln) {
        ...
        $http({
                ...
                data: JSON.stringify({ 'or': ln})
                    ...
                    $scope.creditos = $scope.creditos.filter((el) => {return el.linha != ln.linha } );
    ...
    $scope.saveInfoFin = function (ic, f, c) {
        ...
        if($scope.creditos){
            ...
            if(result.length>=0){
                result.forEach(function (ln){
                   ln.liquidar==0 ? totalOCPrestacoesCC += +ln.prestacao: null;
                   totalOCPrestacoesCP += +ln.prestacao;
                });
            }
        }
        if($scope.s.tipocredito == 'CC'){
            $scope.txEsforco = ($custosGerais + totalOCPrestacoesCC) / $vencimento *100;
            $scope.ic.outroscreditos = totalOCPrestacoesCC;
        } else {
            $scope.txEsforco = ($custosGerais + totalOCPrestacoesCP) / $vencimento *100;
            $scope.ic.outroscreditos = totalOCPrestacoesCP;
        }
    ...
    $scope.checkParceiros = function () {
        $scope.calcularTxEsforco();
         ...
         if (!$scope.p.segundoproponente || $scope.p.segundoproponente == 0) {
            $scope.RLiq = +$scope.p.vencimento + +$scope.calculos.outrosR;
            $scope.RLiqCt = +$scope.p.venc_cetelem + +$scope.calculos.outrosR;             
         } else {
            $scope.RLiq = +$scope.p.vencimento + +$scope.p.vencimento2 + +$scope.calculos.outrosR;
            $scope.RLiqCt = +$scope.p.venc_cetelem + +$scope.p.venc_cetelem2 + +$scope.calculos.outrosR;
         }  
