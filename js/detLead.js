...
angular.module('appGest').controller('detLeadController', function ($scope, $http, $routeParams, $modal) {
    ...
    $scope.noAnswer = function (leadId) {
        ...
        if (!($scope.dl.status == 8 || ($scope.dl.status >= 10 && $scope.dl.status < 36))) {

            if (confirm('Pretende agendar para o dia seguinte? ')) {
                $http({
                    url: 'php/gestor/agendamentoAutomatico.php',
                    method: 'POST',
    ...
    $scope.saveProcesso = function (p) {
        ...
        $http({
            'url': 'php/gestor/saveProcessForm.php',
            'method': 'POST',
            'data': JSON.stringify({'lead': $scope.dl, 'process': p, 'ic': $scope.ic})
        }).then(function (answer) {
            if (answer.data == 1) {
...
//                alert("Guardado");
    ...
    $scope.saveInfoFin = function (ic, f, p) {
        ...
        $scope.p ? ic.diaprestacao = $scope.p.diaprestacao : ic.diaprestacao = 1;
        ...
        $http({
            url: "php/gestor/saveInfoFin.php",
            method: "POST",
        ...
        if ($scope.lead) {
            ...
            $http({
                ...
                if (answer.data.processo) {
                    ...
                    $scope.p.vencimento = $scope.ic.vencimento;
                    $scope.p.vencimento2 = $scope.ic.vencimento2;
                    $scope.p.telefone = $scope.ic.telefone;
                    $scope.p.email = $scope.ic.email;
                    $scope.p.idade = $scope.ic.idade;
                    $scope.p.nif = $scope.ic.nif;
                }

                console.log($scope.p.telefone);
                console.log($scope.p.email);
 ...
 */
angular.module('appGest').controller('modalInstancePedirDoc', function ($scope, $rootScope, $http, $modalInstance, items) {
    $scope.lead = items;
    ...
    //Pedir a documentação selecionada (1)
    $scope.enviarPedidoDoc = function (d, m, e) {
        var docFalta = {};
        docFalta.docs1 = d['docs1'];
        docFalta.docs2 = d['docs2'];
        if (m.dataExpectavel) {
            if (d) {
                $http({
                    url: "php/gestor/sendEmailMissingDocsA_1.php",
                    method: 'POST',
                    data: JSON.stringify({'lead': $scope.lead, 'tipo': '1', 'docFalta': docFalta, 'outroDoc': $scope.outroDoc, 'user': JSON.parse(sessionStorage.userData)})
                }).then(function (answer) {
                    alert(answer.data);
    ...
    //Enviar o pedido da documentação em falta e apenas a que está em falta (2)
    $scope.enviarPedidoDocEmFalta = function (d, m, e) {
        var docFalta = {};
        docFalta.docs1 = d['docs1'];
        docFalta.docs2 = d['docs2'];
        if (m.dataExpectavel) {
            if (d) {
                $http({
                    url: "php/gestor/sendEmailMissingDocsA_1.php",
                    method: 'POST',
                    data: JSON.stringify({'lead': $scope.lead, 'tipo': '2', 'user': JSON.parse(sessionStorage.userData)})
                }).then(function (answer) {
                    alert(answer.data);
                    $modalInstance.close('OK');
                });
    ...
    //Enviar o pedido da documentação em falta e a selecionada (3)
    $scope.enviarPedidoDocEmFaltaPlus = function (d, m, e) {
        var docFalta = {};
        docFalta.docs1 = d['docs1'];
        docFalta.docs2 = d['docs2'];
        if (m.dataExpectavel) {
            if (d) {
                $http({
                    url: "php/gestor/sendEmailMissingDocsA_1.php",
                    method: 'POST',
                    data: JSON.stringify({'lead': $scope.lead, 'tipo': '3', 'docFalta': docFalta, 'outroDoc': $scope.outroDoc, 'user': JSON.parse(sessionStorage.userData)})
                }).then(function (answer) {
                    alert(answer.data);
                    $modalInstance.close('OK');
                });
 ...
 */
angular.module('appGest').controller('modalInstanceViewDoc', function ($scope, $http, $modalInstance, items, $timeout, $rootScope, $sce) {
 ...
 */
angular.module('appGest').controller('modalInstancePedirDoc2', function ($scope, $modalInstance, $http, items) {
    $scope.lead = items;
    $scope.m = {};
    $scope.e = {};
    $scope.e.tipoenvio = "email";
    ...
    $http({
        ...
        $scope.enviarPedidoDoc = function (d) {
            ...
            if (d) {
                $http({
                    url: "php/gestor/sendEmailMissingDocsA_1.php",
                    method: 'POST',
                    data: JSON.stringify({'lead': $scope.lead, 'tipo': '1', 'docFalta': d.docs, 'outroDoc': $scope.outroDoc, 'user': JSON.parse(sessionStorage.userData)})
                }).then(function (answer) {
                    alert(answer.data);
                    $modalInstance.close('OK');
                });
        ...
        //Enviar o pedido da documentação em falta e apenas a que está em falta
        $scope.enviarPedidoDocEmFalta = function (lead, d) {
            ...
            if (d) {
                $http({
                    url: "php/gestor/sendEmailMissingDocsA_1.php",
                    method: 'POST',
                    data: JSON.stringify({'lead': lead, 'tipo': '2', 'user': JSON.parse(sessionStorage.userData)})
                }).then(function (answer) {
                    alert(answer.data);
                    $modalInstance.close('OK');
                });
        ...
        //Enviar o pedido da documentação em falta e a selecionada
        $scope.enviarPedidoDocEmFaltaPlus = function (lead, d) {
            console.log(d);
            if (d) {
                $http({
                    url: "php/gestor/sendEmailMissingDocsA_1.php",
                    method: 'POST',
                    data: JSON.stringify({'lead': lead, 'tipo': '3', 'docFalta': d.docs, 'outroDoc': $scope.outroDoc, 'user': JSON.parse(sessionStorage.userData)})
                }).then(function (answer) {
                    alert(answer.data);
                    $modalInstance.close('OK');
                });
 ...
 */
angular.module('appGest').controller('modalInstanceAgendamentoRec', function ($scope, $http, $modalInstance, items) {
