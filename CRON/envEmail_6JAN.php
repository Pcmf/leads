...
if ($result01) {
    while ($row01 = mysqli_fetch_array($result01, MYSQLI_ASSOC)) {
        ...
        $result = mysqli_query($con, sprintf("SELECT L.* FROM arq_leads L "
                        ...
                        . " WHERE L.user=%s AND YEAR(L.datastatus)=2019 AND MONTH(L.datastatus)=12 AND (L.status=9 "
                        . " OR (L.status=4 AND R.motivo LIKE 'Cancelado a pedido do cliente') )", $user));
        if ($result) {
...
function sendEmailUltimaOportunidade($con, $row) {
    $assunto = "(U6JAN)Ref: " . $row['id'] . " - Ultima Oportunidade para o seu Financiamento!";
    ...
    $nomeGestor = $row0['nome'];
            ...
            $msg = "<p>Olá ".$nomeCliente."</p>"
                    ...
                    . "<p>Trabalhamos com marcas como <strong>Cofídis, Cetelem, Novo Banco, Montepio, Credibom e Unicre.</strong> "
