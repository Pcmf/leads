<?php
/* 
 * Save Created on location LEAD/process with attached docs base64 and change status to 10 or 11.
 */
...
isset($p->outrainfo)?$outrainfo=$p->outrainfo:$outrainfo='';
...
$query = sprintf("INSERT INTO arq_leads(idleadorig,nomelead,fornecedor,tipo,nome,email,telefone,idade,nif,montante,dataentrada,status,datastatus,info,user)" 
        . " VALUES('0','%s',0,'%s','%s','%s','%s',%s,%s,%s,NOW(),%s,NOW(),'%s',%s) ",$p->nomelead,$p->tipo,$p->nome,$p->email,$p->telefone,$p->idade,$p->nif,
        $p->montante,$status,$outrainfo,$userId );
...
if($result0){
    ...
    $query = sprintf("INSERT INTO arq_processo(lead,user,nome,nif,email,telefone,idade,profissao,vencimento,tipocontrato,anoinicio,"
            ...
            . " valorpretendido,prazopretendido,prestacaopretendida,finalidade,outrainfo,moradarua,moradalocalidade,moradacp,tipoenviodoc,datainicio, mesinicio, mesinicio2) "
            . " VALUES(%s,%s,'%s',%s,'%s','%s',%s,'%s',%s,%s,%s,%s,%s,'%s','%s',%s,%s,'%s',%s,%s,%s,'%s',%s,%s,'%s',%s,"
            . " %s,%s,'%s',%s,'%s',%s,%s,%s,'%s','%s','%s','%s','%s','%s',NOW(), %s,%s)",
            $p->id,$userId,$p->nome,$p->nif,$p->email,$p->telefone,$p->idade,$p->profissao,$p->vencimento,$p->tipocontrato->id,$p->anoinicio,
            ...
            $p->valorprestacao,$p->finalidade,$p->outrainfo,$p->moradarua,$p->moradalocalidade,$p->moradacp,'na', $p->mesinicio, $p->mesinicio2);
    $result = mysqli_query($con,$query);
    if($result){
        ...
        foreach ($files as $line){
            ...
            $queryDOC = sprintf("INSERT INTO arq_documentacao(lead,linha,nomefx,tipo,fx64) "
                . " VALUES(%s,%s,'%s','%s','%s')",$p->id,$ln,$line->filename,substr($line->filetype,strpos($line->filetype,"/")+1),$line->base64);
            mysqli_query($con,$queryDOC);
            $ln++;
        }        
