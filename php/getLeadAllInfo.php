...
// Missing Doc or not aproved
$query = sprintf("SELECT SUM(recebido) AS recebido, SUM(aproved) AS aprovado, SUM(notok) AS erro, count(*) AS pedido "
         ."  FROM `cad_docpedida` WHERE lead=%s", $dt->lead);
$result = mysqli_query($con, $query);
if ($result) {
    $temp = array();
    while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
...
$query = sprintf("SELECT * FROM cad_simulag WHERE lead=%s", $dt->lead);
$result = mysqli_query($con, $query);
if ($result) {
    $temp = array();
    while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
...
if ($result->num_rows == 1) {
    ...
    !isset($resp['infoCliente']['estadocivil']) ? $resp['infoCliente']['estadocivil'] = null : null;
    ...
    !isset($resp['infoCliente']['segundoproponente']) ? $resp['infoCliente']['segundoproponente'] = 0 : null;
    !isset($resp['infoCliente']['nome2']) ? $resp['infoCliente']['nome2'] = null : null;
    ...
    !isset($resp['infoCliente']['telefone']) ? $resp['infoCliente']['telefone'] = null : null;
    !isset($resp['infoCliente']['email']) ? $resp['infoCliente']['email'] = null : null;
    !isset($resp['infoCliente']['telefone2']) ? $resp['infoCliente']['telefone2'] = null : null;
    !isset($resp['infoCliente']['tipocontrato']) ? $resp['infoCliente']['tipocontrato'] = 0 : null;
    !isset($resp['infoCliente']['profissao']) ? $resp['infoCliente']['profissao'] = null : null;
    !isset($resp['infoCliente']['tipocontrato2']) ? $resp['infoCliente']['tipocontrato2'] = 0 : null;
    !isset($resp['infoCliente']['profissao2']) ? $resp['infoCliente']['profissao2'] = null : null;
    !isset($resp['infoCliente']['anoinicio']) ? $resp['infoCliente']['anoinicio'] = null : null;
    !isset($resp['infoCliente']['mesinicio']) ? $resp['infoCliente']['mesinicio'] = null : null;
    !isset($resp['infoCliente']['anoinicio2']) ? $resp['infoCliente']['anoinicio2'] = null : null;
    !isset($resp['infoCliente']['mesinicio2']) ? $resp['infoCliente']['mesinicio2'] = null : null;
    ...
    $query = sprintf("INSERT INTO arq_process_form( lead, nome, estadocivil, filhos, nif, segundoproponente, nome2, nif2, relacaofamiliar, "
            ...
            $resp['infoCliente']['segundoproponente'], $resp['infoCliente']['nome2'], $resp['infoCliente']['nif2'], $resp['infoCliente']['relacaofamiliar'],
            $resp['infoCliente']['tipohabitacao'], $resp['infoCliente']['anoiniciohabitacao'], $resp['infoCliente']['tipohabitacao2'], $resp['infoCliente']['anoiniciohabitacao2'],
...
// Obter o speedup
$result = mysqli_query($con, sprintf("SELECT * FROM cad_speedup WHERE lead=%s", $dt->lead));
if ($result->num_rows == 1) {
    $row = mysqli_fetch_array($result, MYSQLI_ASSOC);
    $resp['speedup'] = $row;
}
...
function checkClientOpenLeads($lead, $nif, $email, $telefone, $con) {
    $list = array();
    $query = sprintf("SELECT L.id,L.nome,L.dataentrada,L.nif,L.email,L.telefone,S.nome AS status,L.datastatus,"
            . " U.nome AS usernome,U1.nome AS analista,L.montante "
            . " FROM arq_leads L "
            . " INNER JOIN cnf_statuslead S ON S.id=L.status"
