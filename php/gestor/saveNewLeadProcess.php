...
$query = sprintf("INSERT INTO arq_leads(idleadorig,nomelead,fornecedor,tipo,nome,email,telefone,idade,nif,montante,dataentrada,status,datastatus,info,user)" 
        . " VALUES('0','%s',0,'%s','%s','%s','%s',%s,%s,%s,NOW(),8,NOW(),'%s',%s) ",$p->nomelead,$p->tipo, ucwords( mb_strtolower( $p->nome, 'UTF-8')), 
...
//if($result0){
    ...
    if($result){
        ...
        $queryL = sprintf("UPDATE arq_leads SET status=8, datastatus=NOW(),user=%s WHERE id=%s",$userId,$p->id);
