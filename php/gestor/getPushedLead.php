    ...
    if ($resultA) {
        ...
        if ($rowA[0] > 0) { //Existe Lead agendada, 
            ...
            if (!$resp['call']) {
                ...
                mysqli_query($con, sprintf("UPDATE cad_fila SET status = 0 WHERE lead =%s", $rowA['id']));
...
function puxaLeadNova($dt, $con, $resp) {
    ...
    $result = mysqli_query($con, "SELECT lead FROM cad_fila WHERE status=1 ORDER BY lead,data ASC LIMIT 1");
    if ($result) {
