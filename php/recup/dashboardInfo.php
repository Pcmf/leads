...
$resp = array();
//Novas
$result = mysqli_query($con, sprintf("SELECT COUNT(*) FROM arq_recuperacao "
        . " WHERE statusrec=1"));
...
//Ativa
$result = mysqli_query($con, sprintf("SELECT COUNT(*) FROM arq_leads WHERE status=101 OR (status=102 AND user=%s)",$dt->id));
if($result){
    $row = mysqli_fetch_array($result,MYSQLI_NUM);
    $resp['ativa'] = $row[0];
}
...
//Atribuidas 
$result = mysqli_query($con,"SELECT COUNT(*) FROM arq_leads WHERE status IN (102,106,107,108) AND user=".$dt->id);
if($result){
    $row = mysqli_fetch_array($result,MYSQLI_NUM);
    $resp['ativas'] = $row[0];
}
...
$result = mysqli_query($con, sprintf("SELECT COUNT(*) FROM cad_agendadoc D"
        ...
        . " WHERE  L.user=%s AND L.status =108 AND D.ativa=1 AND D.data<=DATE(NOW())  ORDER BY D.lead ",$dt->id)); 
if($result){
    $row = mysqli_fetch_array($result,MYSQLI_NUM);
...
$result = mysqli_query($con,"SELECT COUNT(*) FROM arq_leads L"
        ...
        . " WHERE L.status=108 AND A.status=1 AND A.tipoagenda=3 AND DATE(A.agendadata)>=DATE(NOW()) AND L.user=".$dt->id);
if($result){
    $row = mysqli_fetch_array($result,MYSQLI_NUM);
...
//Anuladas 
$result = mysqli_query($con,"SELECT COUNT(*) FROM arq_leads WHERE status IN (103,104,105,109) AND DATEDIFF(NOW(), datastatus) < 60  AND user=".$dt->id);
if($result){
    $row = mysqli_fetch_array($result,MYSQLI_NUM);
    $resp['anuladas'] = $row[0];
}
...
$result = mysqli_query($con,"SELECT COUNT(*) FROM arq_leads WHERE status=104 AND DATEDIFF(NOW(), datastatus) < 60 AND user=".$dt->id);
if($result){
    $row = mysqli_fetch_array($result,MYSQLI_NUM);
...
$result = mysqli_query($con,"SELECT COUNT(*) FROM arq_leads WHERE status=105 AND DATEDIFF(NOW(), datastatus) < 60 AND user=".$dt->id);
if($result){
    $row = mysqli_fetch_array($result,MYSQLI_NUM);
...
$result = mysqli_query($con,"SELECT COUNT(*) FROM arq_leads WHERE status=109 AND DATEDIFF(NOW(), datastatus) < 60 AND user=".$dt->id);
if($result){
    $row = mysqli_fetch_array($result,MYSQLI_NUM);
