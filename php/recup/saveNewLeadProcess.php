...
$tipoEnv = $dt->tipoEnv;
$a = $dt->address;
!isset($p->outrainfo)?$outrainfo='':$outrainfo=$p->outrainfo;
!isset($a->email)? $a->email='':null;
...
$query = sprintf("INSERT INTO arq_leads(idleadorig,nomelead,fornecedor,tipo,nome,email,telefone,idade,nif,montante,dataentrada,status,datastatus,info,user)" 
        . " VALUES('0','%s',0,'%s','%s','%s','%s',%s,%s,%s,NOW(),108,NOW(),'%s',%s) ",$p->nomelead,$p->tipo, ucwords( mb_strtolower( $p->nome, 'UTF-8')), 
        $a->email, $p->telefone, $p->idade,$p->nif,  $p->montante,$outrainfo,$userId );
$result =mysqli_query($con,$query);

if($result){
$p->id = mysqli_insert_id($con);

    if(!isset($a->moradarua)){$a->moradarua='';}
    if(!isset($a->moradalocalidade)) {$a->moradalocalidade='';}
    if(!isset($a->moradacp)) {$a->moradacp='';}
    if(!isset($p->filhos) || !$p->filhos) {$p->filhos=0;}
    if(!isset($p->valorhabitacao)) {$p->valorhabitacao=0;}    
    if(!isset($p->declarada)) {$p->declarada='';}    
    if(!isset($p->prazopretendido) || !$p->prazopretendido) {$p->prazopretendido=0;}
    if(!isset($p->valorprestacao) || !$p->valorprestacao) {$p->valorprestacao=0;}
    if(!isset($p->outrainfo)) {$p->outrainfo='';}
    if(!isset($p->finalidade)) {$p->finalidade='';}
    !isset($p->segundoproponente) || !$p->segundoproponente ? $p->segundoproponente=0 : $p->segundoproponente=1;
    if(!isset($p->profissao2)) {$p->profissao2='';}
    if(!isset($p->idade2) || !$p->idade) {$p->idade2='NULL';}    
    if(!isset($p->vencimento2) || !$p->vencimento2) {$p->vencimento2='NULL';} 
    
    if(!isset($p->valorhabitacao2) || !$p->valorhabitacao2) {$p->valorhabitacao2=0;}
    if(!isset($p->anoiniciohabitacao2) || !$p->anoiniciohabitacao2) {$p->anoiniciohabitacao2=1900;}
    if(!isset($p->declarada2)) {$p->declarada2='';}
    if(!isset($p->parentesco2)) {$p->parentesco2='';}
    !isset($p->tipohabitacao2) || !$tipohabitacao2 ? $tipohabitacao2='NULL': $tipohabitacao2=$p->tipohabitacao2->id;    
    ...
    !isset($p->tipocontrato2) || !$tipocontrato2 ? $tipocontrato2='NULL': $tipocontrato2=$p->tipocontrato2->id;   
    if(!isset($p->anoinicio2) || !$p->anoinicio2) {$p->anoinicio2='NULL';} 
    if(!isset($p->telefone2)) {$p->telefone2='';} 
    if(!isset($p->nif2) || !$p->nif2) {$p->nif2='NULL';}     
    ...
    $query = sprintf("INSERT INTO arq_processo(lead,user,nome,nif,email,telefone,idade,profissao,vencimento,tipocontrato,anoinicio,"
            ...
            $p->id,$userId, ucwords( mb_strtolower( $p->nome, 'UTF-8')), $p->nif,$p->email,$p->telefone,$p->idade,$p->profissao,$p->vencimento,$p->tipocontrato->id,$p->anoinicio,
            ...
            $p->valorprestacao,$p->finalidade,$p->outrainfo,$a->moradarua,$a->moradalocalidade,$a->moradacp,$dt->tipoEnv,
            $p->tipocredito, $p->mesinicio, $p->mesinicio2, $p->segundoproponente);
    $result = mysqli_query($con,$query);
    if($result){
        ...
        $queryAG = sprintf("INSERT INTO cad_agenda(lead,user,data,agendadata,agendahora,agendaperiodo,tipoagenda,status) "
                ...
                $p->id, $userId,$data1,0,0,3,1);
        mysqli_query($con,$queryAG);
        
        //Alterar o status da LEAD na tabela LEADS para a aguardar documentação
        $queryL = sprintf("UPDATE arq_leads SET status=108, datastatus=NOW(),user=%s WHERE id=%s",$userId,$p->id);
    ...
    //ENVIO de EMAIL para pedir documentação
            ...
            //Assunto
            $assunto = "Ref:".$p->id." - Obrigado. Documentação em falta.";
            ...
            . "<p> No seguimento do pedido anterior, venho por este meio solicitar o envio da seguinte documentação,"
            . " que ficou em falta:</p>"
            . " ".$lista." <br/><br/>"
            ...
            ."<p>Ao usar o Área do Cliente para anexar a documentação pedida estará a dar mais rapidez ao seu processo!</p>";
    

    
         //Enviar o email
        $result = new sendEmail($con,$dt->user->id, $dt->user->email, $p->email, $assunto, $msg,"", 10, $p->id);        
        if($result){
