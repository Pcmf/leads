...
if($dt->sts == 3){
    $query  = sprintf("SELECT P.lead,P.nome,P.telefone,P.nif,P.email,DATE(P.datainicio) AS dtinicial,D.assunto,D.dataentrada, F.nome AS fornecedornome "
            ...
            . " WHERE L.status=109  AND A.tipoagenda=3 AND A.status=1 AND P.user = %s GROUP BY P.lead ORDER BY P.lead",$dt->userId);
}
//Aguarda
if($dt->sts == 1 || $dt->sts==21){
    $query  = sprintf("SELECT P.lead,P.nome,P.telefone,P.nif,P.email,DATE(P.datainicio) AS dtinicial,A.agendadata, F.nome AS fornecedornome "
            ...
            . " INNER JOIN cad_agenda A ON P.lead = A.lead "
            . " INNER JOIN cad_fornecedorleads F ON F.id=L.fornecedor "
            . " WHERE L.status =108 AND A.status=1 AND A.tipoagenda=3 AND DATE(A.agendadata)>=DATE(NOW()) AND L.user = %s GROUP BY P.lead ORDER BY P.lead",$dt->userId);
}
//Aguarda
if($dt->sts==21){
...
if($dt->sts == 10){
    $query = sprintf("SELECT P.lead,P.nome,P.telefone,P.nif,P.email,DATE(P.datainicio) AS dtinicial,A.agendadata, F.nome AS fornecedornome "
        ...
        . " INNER JOIN cad_fornecedorleads F ON F.id=L.fornecedor "    
        . " WHERE L.status=108 AND A.status=1 AND A.tipoagenda=3 AND DATE(A.agendadata)<DATE(NOW()) AND L.user=%s  ORDER BY P.lead",$dt->userId);
}
//atrasados
if($dt->sts == 101){
