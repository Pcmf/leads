...
if ($result0) {
    ...
    $simulacao = '';
    // Outras simulações
    $resultSim = mysqli_query($con, sprintf("SELECT * FROM cad_simula WHERE lead =%s ORDER BY data DESC LIMIT 1", $dt->lead));
    if ($resultSim) {
        $ln = 1;
        $line = mysqli_fetch_array($resultSim, MYSQLI_ASSOC);
        $simulacao .= "<p><strong>Valor de simulação " . $ln . " ) </strong> &nbsp;&nbsp;&nbsp;<em>"
                    . "Valor pretendido: <strong>" . $line['valorpretendido'] . " Euros</strong></em>"
                    . " &nbsp;&nbsp;&nbsp;&nbsp;<em>Prazo: <strong>" . $line['prazopretendido'] . " Meses</strong></em>"
                    . " &nbsp;&nbsp;&nbsp;&nbsp;<em>Prestação: <strong>" . $line['prestacaopretendida'] . " Euros</strong></em></p>";
        // Outras simulações
