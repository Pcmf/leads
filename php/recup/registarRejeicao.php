...
$dt->motivo->motivoComum == "Falta de Documentação" ? $status=109 : $status = 104;
...
//Enviar o email para o cliente
    ...
    if($result0){
       ...
       $dt->motivo->motivoComum=='outro motivo a registar' ?  $motivoComum = null :  "<li>".$motivoComum = $dt->motivo->motivoComum."</li>";
        ...
        $assunto = "Ref: ".$id." - Anulação da proposta.";
        $sujestaoCC = '';
//        if ($dt->motivo->motivoComum == 'Taxa de esforço elevada') {

        if ($dt->motivo->motivoComum != 'Incidentes Bancários' 
                    && $dt->motivo->motivoComum != 'Titulo de residência temporário'
                    &&  $dt->motivo->motivoComum != 'Não declara rendimentos' ) {
        ...
        //Enviar o email
        $result = new sendEmail($con, $dt->user->id, $dt->user->email, $row['email'],$assunto, $msg, $anexo, 7, $id);        
        if($result){
