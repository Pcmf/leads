...
for ($i = 0; $i < 4; $i++) {
    ...
    for ($k = $dt->mes1; $k <= $dt->mes2; $k++) {
        ...
        $query = sprintf("SELECT count(distinct(L.id)) AS QTY "
                ...
                . " AND ( P.vencimento BETWEEN %s AND %s) "
                . " %s %s",
                $dt->ano, $k, $dt->gv[$i*2], $dt->gv[$i*2+1]  ,$selectbyEmpresa, $tipocreditoL);
...
for ($i = 0; $i < 4; $i++) {
    ...
    for ($k = $dt->mes1; $k <= $dt->mes2; $k++) {
        ...
        $query = sprintf("SELECT COUNT(distinct(L.id)) AS QTY, SUM(FI.montante) AS VALOR"
                ...
                . " LEFT JOIN arq_processo P ON P.lead=L.id "
                . " WHERE YEAR(L.dataentrada)=%s AND MONTH(L.dataentrada) = %s "
                . " AND P.vencimento BETWEEN %s AND %s AND FI.status=7 "
                . " %s %s ",
                $dt->ano, $k, $dt->gv[$i*2], $dt->gv[$i*2+1], $selectbyEmpresa, $tipocreditoFI);
        $result = mysqli_query($con, $query);
        if ($result) {
            $row = mysqli_fetch_array($result, MYSQLI_ASSOC);
