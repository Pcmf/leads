...
angular.module('appMain').controller('estAnalistController',function($scope,$http){
    $scope.tml={};
    $scope.tml.opc = 'mes';
    getInfo($scope.tml);
    //Analise
    ...
    function getInfo(tml){
            ...
            $scope.timeLine = tml;
            $http({
                url:'statistic/getStatisticsByAnalist.php',
                method:'POST',
                data:JSON.stringify(tml)
            }).then(function(answer){
                $scope.byAnalist = answer.data.dados;
                $scope.paraanalise = answer.data.paraanalise;
                $scope.openparaanalise = answer.data.openparaanalise;
            });
    };
